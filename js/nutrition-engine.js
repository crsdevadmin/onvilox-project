...
function buildFormulationOptions(targets) {
  ...
  const comorbiditiesObj = Array.isArray(patient.comorbidities) ? patient.comorbidities : [];
  const isDiabeticCarb = comorbiditiesObj.some(c => c.toLowerCase().includes('diabetes')) || bloodSugar > 100;

  let selectedCarb = IngredientLibrary.find(i => i.id === 'palatinose');
  if (!isDiabeticCarb && !cachexia) {
